...
ifeq ($(SGX_DEBUG), 1)
        SGX_COMMON_FLAGS += -O0 -g #here need to do some modification
else
        SGX_COMMON_FLAGS += -O2
endif
...
Enclave_Link_Flags := $(Enclave_Security_Link_Flags) \
    -Wl,--no-undefined -nostdlib -nodefaultlibs -nostartfiles -L$(SGX_LIBRARY_PATH) -L$(SGXSSL_LIB_PATH) \
	-Wl,--whole-archive -l$(Trts_Library_Name) -Wl,--no-whole-archive \
	-Wl,--whole-archive -lsgx_tsgxssl -Wl,--no-whole-archive -lsgx_tsgxssl_crypto -lsgx_pthread \
...
target: $(App_Name) $(Signed_Enclave_Name) App/app.so
...
App/app.so: App/Enclave_u.o $(App_Cpp_Objects)
