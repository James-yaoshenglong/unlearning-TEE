...
class PackedTable {
 ...
 public:
  ...
  void PrintBucket(const size_t i) const {
    ...
    //           << std::endl;
    printf("\tbucketbits  =%s\n", PrintUtil::bytes_to_hex((char *)p, kBytesPerBucket + 1).c_str());
  ...
  void PrintTags(uint32_t tags[4]) const {
    ...
    for (size_t j = 0; j < 4; j++) {
      ...
      printf("\ttag[%d]: %s", PrintUtil::bytes_to_hex((char *)&tags[j], 4).c_str());
      // std::cout << " lowbits="
      ...
      //                                      kDirBitsPerTag / 8 + 1)
      //           << std::endl;
      printf(" lowbits=%s dirbits=%s\n", PrintUtil::bytes_to_hex((char *)&lowbits[j], 1).c_str(), PrintUtil::bytes_to_hex((char *)&dirbits[j],kDirBitsPerTag / 8 + 1).c_str());
    }
  ...
  bool InsertTagToBucket(const size_t i, const uint32_t tag, const bool kickout,
    ...
    if (kickout) {
      size_t r;
      sgx_read_rand((unsigned char *)&r, sizeof(size_t));
      r = r & 3;
      DPRINTF(
          DEBUG_TABLE,
